...
# 计算两个框的特定IOU
# def bbox_iou(box1, box2, x1y1x2y2=True, GIoU=False, DIoU=False, CIoU=False, SIoU=False, EIoU=False, Focal=False, alpha=1, gamma=0.5, eps=1e-7):
#     # Returns the IoU of box1 to box2. box1 is 4, box2 is nx4
...
#             # rho2 = ((torch.abs((b2_x1 + b2_x2 - b1_x1 - b1_x2)) + torch.abs((b2_y1 + b2_y2 - b1_y1 - b1_y2)) / 4)) ** alpha  # center dist ** 2
#
#             if CIoU:  # https://github.com/Zzh-tju/DIoU-SSD-pytorch/blob/master/utils/box/box_utils.py#L47
#                 v = (4 / math.pi ** 2) * (torch.atan(w2 / h2) - torch.atan(w1 / h1)).pow(2)
#                 with torch.no_grad():
...
#                 rho_w2 = ((b2_x2 - b2_x1) - (b1_x2 - b1_x1)) ** 2
#                 rho_h2 = ((b2_y2 - b2_y1) - (b1_y2 - b1_y1)) ** 2
#                 # rho_w2 = torch.abs((b2_x2 - b2_x1) - (b1_x2 - b1_x1))
#                 # rho_h2 = torch.abs((b2_y2 - b2_y1) - (b1_y2 - b1_y1))
#                 cw2 = torch.pow(cw ** 2 + eps, alpha)
#                 ch2 = torch.pow(ch ** 2 + eps, alpha)
#                 if Focal:
...
#                 rho_x = (s_cw / cw) ** 2
#                 rho_y = (s_ch / ch) ** 2
#                 # rho_x = torch.abs(s_cw) / cw
#                 # rho_y = torch.abs(s_ch) / ch
#                 gamma = angle_cost - 2
#                 distance_cost = 2 - torch.exp(gamma * rho_x) - torch.exp(gamma * rho_y)
#                 omiga_w = torch.abs(w1 - w2) / torch.max(w1, w2)
#                 omiga_h = torch.abs(h1 - h2) / torch.max(h1, h2)
...
# def bbox_iou(box1, box2, x1y1x2y2=True, GIoU=False, DIoU=False, CIoU=False, SIoU=False, EIoU=False, Focal=False, alpha=3, gamma=0.5, eps=1e-9):
#     # Returns Intersection over Union (IoU) of box1(1,4) to box2(n,4)
#     box2 = box2.T
#     # Get the coordinates of bounding boxes
#     if x1y1x2y2:  # transform from xywh to xyxy
